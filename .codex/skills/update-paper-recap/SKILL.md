...
7. Create or edit exactly one Markdown note in `content/papers/`. Name new files `YYYY-MM-DD-short-kebab-slug.md`, using the read date and a stable English slug. Set `read_at` on every new note to the actual local reading/submission time in timezone-aware ISO 8601 format so same-day entries sort correctly. When merging an existing note, preserve its original `read_at` unless the user explicitly records a new reading session. Add that note's figures (see step 10) under `content/media/<slug>/`.
...
10. Actively look for a good figure for every entry — a method/architecture overview or one decisive result figure is usually the single highest-value recall aid a note can have. Do not skip this step merely because sourcing a figure takes extra work. When the source is a PDF, use the PDF skill to render and visually inspect the relevant pages, then crop only the meaningful figure. When the source is a blog, project page, or repo, fetch the page and pull the relevant diagram/screenshot the same way. Optimize the crop without sacrificing labels or legibility. Verify reuse permission because the site is public; include figure number (or a descriptive name when the source has none), source, and license in the Markdown caption. Only skip a figure when no image in the source materially helps recall, or reuse permission is genuinely unclear after checking — state explicitly in your response to the user which of these applied; never skip silently.
